...
! function ($) {
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        html.push('</div>',
            '<div style="cursor:not-allowed" class="pull-' + this.options.paginationHAlign + ' pagination">',
    ...
    BootstrapTable.prototype.initServer = function (silent, query) {
        ...
        request = $.extend({}, calculateObjectValue(null, this.options.ajaxOptions), {
            ...
            success: function (res) {
                /**Mid 2017-12-22 Evan新加的代码,处理页码错误问题开始*/
                if (that.options.pagination && res.totalCount && !res.result.length) {//总记录数大于0,但当前页记录数为0,则此时页码超过了最大页码误
                    that.options.pageNumber = Math.ceil(res.totalCount / that.options.pageSize);//最后一页(总页数)
                    that.initServer();
                    return;
                }
